...
Detailed Description: <one continuous shot: GEO, first-frame blocking, subject action, composition, exactly one camera movement type + amplitude + speed, physics, and visible final state>
...
Reference labels are literal: `<Subject N>`, `<Picture N>`, `<Video N>`, and `<Audio N>`. A reference-retention line must name the useful transfer, such as picture-derived appearance, video-derived hand motion, or audio-derived rhythm. That retention line is the Preserve contract — skip it and identity or product geometry drift. It must not attribute a role to unattached material.
